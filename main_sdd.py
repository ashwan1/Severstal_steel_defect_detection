...
def run(backbone, batch_size, lr, dropout_rate, data_path, artifacts_folder,
        img_size, use_multi_gpu, gpu_count, use_cbam, use_se, do_classification, seed, _run):
    ...
    train_df['has_defect_1'] = (~train_df['defect_1'].isna()).astype(np.int8)
    train_df['has_defect_2'] = (~train_df['defect_2'].isna()).astype(np.int8)
    train_df['has_defect_3'] = (~train_df['defect_3'].isna()).astype(np.int8)
    train_df['has_defect_4'] = (~train_df['defect_4'].isna()).astype(np.int8)
    ...
    if use_multi_gpu:
        ...
        sdd_model = SDDModel(backbone, img_size, lr, dropout_rate, _MODEL_ARC,
                             use_cbam=use_cbam, use_se=use_se).get_model()
    sdd_model.summary()
